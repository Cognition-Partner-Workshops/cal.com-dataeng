...
export default function BranchManagerDashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3" role="group" aria-label="Pipeline summary">
        {pipeline.map(p => (
          <button key={p.status} className="bg-white rounded-lg shadow-sm border p-3 cursor-pointer hover:shadow-md text-left"
            ...
            aria-label={`Filter by ${p.status.replace(/_/g, ' ')}: ${p.count} applications`}>
      ...
      {/* Filter */}
      <label htmlFor="bm-status-filter" className="sr-only">Filter by status</label>
      <select id="bm-status-filter" value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); setPage(1); }}
      ...
      <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
        <table className="w-full text-sm" aria-label="Branch applications">
          <thead className="bg-gray-50 border-b">
            <tr>
              ...
              <th scope="col" className="px-4 py-3 text-left font-medium text-gray-500">Officer</th>
              ...
              <tr key={app.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-center">
                  <button onClick={() => { setReassignModal(app.id); setSelectedOfficer(''); }}
                    ...
                    aria-label={`Reassign application ${app.application_number}`}>
                    <ArrowRightLeft className="w-3 h-3 mr-1" aria-hidden="true" /> Reassign
                  </button>
                </td>
              </tr>
            ))}
      ...
      {/* Reassign Modal */}
      {reassignModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" role="dialog" aria-modal="true" aria-labelledby="reassign-title"
          onClick={() => setReassignModal(null)}
          onKeyDown={(e) => { if (e.key === 'Escape') setReassignModal(null); }}>
          <div className="bg-white rounded-xl p-6 w-96 shadow-2xl" onClick={e => e.stopPropagation()}>
            <h3 id="reassign-title" className="text-lg font-semibold mb-4">Reassign Loan Officer</h3>
            <label htmlFor="reassign-officer" className="sr-only">Select new loan officer</label>
            <select id="reassign-officer" value={selectedOfficer} onChange={(e) => setSelectedOfficer(e.target.value)}
